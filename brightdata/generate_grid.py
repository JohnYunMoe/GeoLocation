...
def generate_grid(min_lat, min_lon, max_lat, max_lon, cell_size_km, keywords, country="AE", zoom=14.5):
    ...
    while lat <= max_lat:
        ...
        while lon <= max_lon:
            ...
            lon += (1/2 * lon_step + lon_step)  # add extra half step to reduce overlap between adjacent cells
        lat += (1/2 * lat_step + lat_step)  # add extra half step to reduce overlap between adjacent cells
